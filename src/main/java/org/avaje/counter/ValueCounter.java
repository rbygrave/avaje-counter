...
public class ValueCounter {
  ...
  public CounterStatistics getStatistics(boolean reset) {
    ...
    if (reset) {
      ...
      return new CounterStatistics(startTime.getAndSet(now), count.sumThenReset(), total.sumThenReset(), max.getAndSet(0));
      
    } else {
      return new CounterStatistics(startTime.get(), count.sum(), total.sum(), max.get());
    }
  }
